...
RSpec.describe Position, type: :model do
  ...
  describe 'バリデーション' do
    ...
    context '必須項目の確認' do
      it 'titleが必須である' do
        position = Position.new(company: company, title: nil, department: '開発部', started_at: Date.today)
        expect(position).to be_invalid
        expect(position.errors[:title]).to include("を入力してください")
      end

      it 'departmentが必須である' do
        position = Position.new(company: company, title: 'エンジニア', department: nil, started_at: Date.today)
        expect(position).to be_invalid
        expect(position.errors[:department]).to include("を入力してください")
      end

      it 'started_atが必須である' do
        position = Position.new(company: company, title: 'エンジニア', department: '開発部', started_at: nil)
        expect(position).to be_invalid
        expect(position.errors[:started_at]).to include("を入力してください")
      end

      it 'companyが紐づいていない場合、作成に失敗する' do
        position = Position.new(company: nil, title: 'エンジニア', department: '開発部', started_at: Date.today)
        expect(position).to be_invalid
    ...
    context 'ended_atの扱い' do
      it 'ended_atがnilでも有効（現在この役職）' do
        position = Position.new(company: company, title: 'エンジニア', department: '開発部', started_at: Date.today, ended_at: nil)
        expect(position).to be_valid
      end

      it 'ended_atが設定されていても有効（過去の役職）' do
        position = Position.new(company: company, title: 'エンジニア', department: '開発部', started_at: Date.new(2020, 1, 1), ended_at: Date.new(2023, 12, 31))
        expect(position).to be_valid
      end
    end
  end
  ...
  describe '作成' do
    ...
    it '役職情報が正常に作成できる' do
      ...
      expect(position.department).to eq('開発部')
